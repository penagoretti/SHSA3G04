...
\begin{enumerate}
  ...
  \item Compute teh $p$-value based on the exact distribution of the data under the null hypothesis, using each of the following methods:
  \begin{enumerate}
...
the $p$-value in an \textsf{R} script, using the proper existing probability function \footnote{
