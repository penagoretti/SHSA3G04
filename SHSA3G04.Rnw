...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item
      \textbf{Step 2.} Carry out the experiment and obtain the data.\\
    In the sample of size $n = \Sexpr{n}$, the number of workers that have acceptable levels of pollutant is $k = \Sexpr{k}$.
    \item \textbf{Step 3.} Compute and interpret a measure of distance related to the hypothesis to be contrasted.\\
    Prevalence of workers that have acceptable levels of pollutant can be estimated\footnote{See Appendix \ref{app:prevalence_estimation}.} as:
...
@
  ...
  \begin{enumerate}
    \item  By hand (i.e. obtaining the mathematical expression for the $p$-value and then computing its numerical value using \textsf{R} as a simple calculator). Show all your derivations and your \textsf{R} code.
      ...
      We know that, under the null hypothesis $H_0$, $\pi=0.95$, thus
      \begin{equation}
      P(X=k)={20\choose k}0.95^k \cdot 0.05^{20-k}
      \end{equation}
...
D = abs(E-k)
@   
Therefore, results at least as favorable to $\text{H}_1$ than those observed are $|X - X_e| \geq |X_o - X_e| = |\Sexpr{k} - \Sexpr{E}| = \Sexpr{D}$, where $X_o$ and $X_e$ are the observed value in the experiment and the expected value under $\text{H}_0$\footnote{See Appendix \ref{app:results}.}, respectively, and the $p$-value is

  \begin{equation}
    p\text{-value}=P(|X-\Sexpr{E}|\geq \Sexpr{D}|H_0)=
    P(|X-\Sexpr{E}|\geq \Sexpr{D}|X\sim\text{Binomial} (n=\Sexpr{n},\pi = 0.95))
  \end{equation}
  Or, equivalently,
  \begin{equation}
  \begin{split}
    p\text{-value}&=P(\{X-\Sexpr{E}\geq \Sexpr{D}\}\cup \{\Sexpr{E}-X\geq \Sexpr{D}\})\\
    &=P(\{X\geq \Sexpr{E+D}\}\cup \{X\leq\Sexpr{E-D}\})\\
    &=P(X\geq \Sexpr{E+D})+ P(X\leq\Sexpr{E-D})\\
    &=1-P(X\leq \Sexpr{E+D})+P(X\leq\Sexpr{E-D})
    \end{split}
  \end{equation}
  where $X\sim\text{Binomial}(n=\Sexpr{n}, \pi=0.95)$. Using the definition of the Cumulative Density Function (or CDF for short), and assuming that X has a Binomial Distribution with the parameters stated above, we know
  \begin{equation}
  P(X \leq k)=\sum_{i=0}^k{\Sexpr{n}\choose i} 0.95^{i}(0.05)^{\Sexpr{n}-i}
  \end{equation}
  being $k$ an integer. Thus,
  \begin{equation}\begin{split}
  p\text{-value}
  &=1-\sum_{i=0}^{20} {\Sexpr{n}\choose i} 0.95^{i}(0.05)^{\Sexpr{n}-i}+
  \sum_{i=0}^{18}{\Sexpr{n}\choose i} 0.95^{i}(0.05)^{\Sexpr{n}-i}\\
...
k1<-20
for(i in 0:k1){
  s1<-s1+nchoosek(n,i)*p^i*p_^(n-i)
}
s2<-0
k2<-18
for(i in 0:k2){
  s2<-s2+nchoosek(n,i)*p^i*p_^(n-i)
}
...
<<>>=
#El p-valor debería dar esto: 
# P(X>=20)+P(X<=18)=0.35848592+0.26416047=0.62
#pero da 0.26. WTF? Q estoy haciendo mal?
@
...
Testing the function with the requested data, we can see the error message:
