...
\begin{enumerate}
  \item State clearly the hypothesis test.\\
        First, we state the hypothesis test and set the significance level.\\
    ...
    Then, we carry out the experiment and obtain the data.\\
    In the sample of size $n = \Sexpr{n}$, the number of workers that have acceptable levels of pollutant is $k = \Sexpr{k}$.\\
    Finally, we are going to compute and interpret a measure of distance related to the hypothesis to be contrasted.\\
